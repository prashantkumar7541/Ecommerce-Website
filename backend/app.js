...
// Multer configuration for file upload to Cloudinary
...
const upload = multer({ storage: storage }).array('images', 5); // Allow up to 5 images per product

// Middleware
app.use(cors({
  origin: ['http://localhost:3000', 'https://yourfrontend.com'], // Add your frontend URLs here
  credentials: true,
  methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
}));
...
// Create Product with Image Upload Route
app.post('/create-product', upload, async (req, res) => {
  try {
...
app.post('/create-coupon', async (req, res) => {
  try {
    ...
    const coupon = new Coupon({ code, discount, expiryDate, isActive: true });
...
app.post('/apply-coupon', async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'Coupon applied successfully', finalAmount });
  } catch (error) {
...
app.post('/place-order', async (req, res) => {
  try {
    const { orderId, userId, date, time, address, email, name, productIds, price } = req.body;
    const order = new Order({ orderId, userId, date, time, address, email, name, productIds, price });
